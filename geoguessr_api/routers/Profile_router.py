...
@router.patch("/")
async def name_edit(request: EditName, token: dict = Depends(dependies.get_current_user)):
    return await profile.NameEdit(token["login"], request.new_name)


@router.put("/avatar")
async def avatar_edit(
    file: UploadFile = Depends(dependies.validate_avatar), token: dict = Depends(dependies.get_current_user)
):
    return await profile.AvatarEdit(token["login"], file)


@router.get("/avatar")
async def avatar_get(token: dict = Depends(dependies.get_current_user)):
    return profile.get_avatar(token["login"])

@router.get("/me")
async def me(token: dict = Depends(dependies.get_current_user)):
